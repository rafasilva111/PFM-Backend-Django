...
from apps.etl_app.models import default_properties
from apps.etl_app.etl.extract.recipe.continente.main import extract_models_ as recipe_continente_extract_models
from apps.etl_app.etl.extract.recipe.continente.models import Recipe as ExtractContinenteRecipe, RecipeLink as ExtractContinenteRecipeLink, database_proxy as recipe_continenete_extract_database_proxy
...
class TaskExtractRecipesContinenteTestCase(TestCase):
    ...
    def test_default_behavior_light(self):
        """
        Test the default extraction behavior for Continente Recipes when no stopping condition is set.

        Workflow:
            1. Prints a prologue for test context.
            2. Runs the extraction process for Continente Recipes using the configured task and database.
            3. Asserts that the number of extracted recipes and recipe links matches the base threshold value.
            4. Asserts that the extraction task step and status are as expected (finished).
            5. Verifies that all extracted recipe links are unique.
            6. Verifies that all extracted recipes are unique by title.

        This test ensures that, in the absence of a threshold stopping condition, the extraction process
        completes all available items and produces unique results.
...
from apps.etl_app.etl.extract.ingredient.continente.main import extract_models_ as ingredient_continente_extract_models, PAGE_LINKS_OFFSET as ingredient_continente_page_links_offset
from apps.etl_app.etl.extract.ingredient.continente.models import Ingredient as ExtractContinenteIngredient, IngredientLink as ExtractContinenteIngredientLink, database_proxy as ingredient_continenete_extract_database_proxy
import re
