...
from apps.etl_app.recipe.transform.models import (
    database_proxy,
    Recipe as Recipe_T,
...
def transform_recipe(logger, task, instance, stopping_offset, db):
    ...
    if check_task_stopping_condition(task, stopping_offset, instance):
    ...
    try:
        with db.atomic():
            ...
            if check_if_task_stopped(task) or check_task_stopping_condition(task, stopping_offset, instance):
                logger.info(f"[Thread {threading.current_thread().name}] Stop or stopping condition detected after processing Recipe ID {instance.id}. Rolling back.")
